...
static const size_t limb128bits =
    (128 + bignum256modm_bits_per_limb - 1) / bignum256modm_bits_per_limb;
...
static void heap_insert_next(batch_heap* heap)
{
  ...
  parent = (node - 1) / 2;
  while (node && lt256_modm_batch(scalars[pheap[parent]], scalars[pheap[node]],
                                  bignum256modm_limb_size - 1))
  {
...
static void heap_updated_root(batch_heap* heap, size_t limbsize)
{
  ...
  {
    node = lt256_modm_batch(scalars[pheap[childl]], scalars[pheap[childr]],
                            limbsize)
               ? childr
               : childl;
    heap_swap(pheap, parent, node);
  ...
  parent = (node - 1) / 2;
  while (node && lte256_modm_batch(scalars[pheap[parent]], scalars[pheap[node]],
                                   limbsize))
  {
    heap_swap(pheap, parent, node);
    node = parent;
...
static void heap_get_top2(batch_heap* heap,
                          heap_index_t* max1,
                          heap_index_t* max2,
                          size_t limbsize)
{
...
/* */
static void ge25519_multi_scalarmult_vartime_final(ge25519* r,
                                                   ge25519* point,
                                                   bignum256modm scalar)
{
  const bignum256modm_element_t topbit =
      ((bignum256modm_element_t)1 << (bignum256modm_bits_per_limb - 1));
...
/* count must be >= 5 */
static void ge25519_multi_scalarmult_vartime(ge25519* r,
                                             batch_heap* heap,
                                             size_t count)
{
  ...
  for (;;)
  {
    ...
    sub256_modm_batch(heap->scalars[max1], heap->scalars[max1],
                      heap->scalars[max2], limbsize);
    ge25519_add(&heap->points[max2], &heap->points[max2], &heap->points[max1]);
    heap_updated_root(heap, limbsize);
  }

  ge25519_multi_scalarmult_vartime_final(r, &heap->points[max1],
                                         heap->scalars[max1]);
}
  ...
  while (num > 3)
  {
    ...
    for (i = 0; i < batchsize; i++)
      if (!ge25519_unpack_negative_vartime(&batch.points[i + 1], pk[i]))
        goto fallback;
    for (i = 0; i < batchsize; i++)
      if (!ge25519_unpack_negative_vartime(&batch.points[batchsize + i + 1],
                                           RS[i]))
        goto fallback;
    ...
    fallback:
      ...
      {
        valid[i] =
            ED25519_FN(ed25519_sign_open)(m[i], mlen[i], pk[i], RS[i]) ? 0 : 1;
        ret |= (valid[i] ^ 1);
      }
    }
